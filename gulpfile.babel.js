...
const PATHS = {
	...
	src_imgs:  './src/public/imgs/',
	src_node : './src/',
	dist_scss: './dist/public/css/',
	dist_js  : './dist/public/js/',
	dist_imgs: './dist/public/imgs/',
	dist_node: './dist/',
}
...
gulp.task('mv-imgs', () => {
	return gulp.src([
				 path.join(PATHS.src_imgs, '*.jpeg'),
				 path.join(PATHS.src_imgs, '*.jpg'),
				 path.join(PATHS.src_imgs, '*.png'),
				 path.join(PATHS.src_imgs, '*.gif')
			 ], { base: './src/public/imgs/' })
			.pipe(gulp.dest(PATHS.dist_imgs))
})
...
gulp.task('default', ['sass', 'babel', 'mv-tmpl', 'mv-imgs', 'babel-server', 'nodemon'], () => {
	bSync.init(null, {
